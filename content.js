...
window.__SA_WING_PICK__=window.__SA_WING_PICK__||(function(){
  ...
  function onMapDown(e){
    ...
    if(e.shiftKey)return; // marquee owns shift-drag
    ...
    if(window.__SA_WING_CONFIRM__)window.__SA_WING_CONFIRM__({wing:order,dest:dest});
  ...
  function start(wingIdOrIds,mode){
    ...
    listenMap();
    logFlight({type:"PICK",wing:label,count:keys.length,mode:state.mode,msg:keys.length+" fleets · click map (no planner pane) · Esc cancel"});
    try{
      ...
      tip.textContent=(state.mode==="warp"?"WARP":"SWARP")+" · "+label+" · "+keys.length+" · CLICK MAP · ESC";
...
window.__SA_ACTION_BAR__=window.__SA_ACTION_BAR__||(function(){
  ...
  function css(){
    var cssTxt=""
+"#sa-action-bar{position:fixed;left:50%;bottom:16px;transform:translateX(-50%);z-index:999990;display:flex;flex-direction:column;align-items:center;gap:8px;pointer-events:none;"
...
+"#sa-combat-log-box .sa-cl-ico:hover{border-color:rgba(255,190,77,.5);color:#ffbe4d;background:rgba(255,190,77,.08)}";
  ...
  function ensure(){
    ...
    root.innerHTML='<div class="sa-cancel-row" data-cancel></div><div class="sa-row" data-wings></div><div class="sa-acts" data-acts></div><div class="sa-tools"><button type="button" data-a="groups">Wings</button><button type="button" data-a="flight">Log</button></div>';
  ...
  function boot(){if(!document.body){setTimeout(boot,200);return}ensure();paint();window.addEventListener("keydown",onKey,true);setInterval(function(){try{paint()}catch(e){}},900)}
  setTimeout(boot,1600);
  return{paint:paint,run:run,openGroups:openGroups,selectWing:selectWing,clearSelection:clearSelection,getActiveWings:function(){return activeWingIds.slice()}};
})();
