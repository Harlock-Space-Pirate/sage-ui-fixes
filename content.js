...
+"#sa-combat-log-box .sa-cl-ico:hover{border-color:rgba(255,190,77,.5);color:#ffbe4d;background:rgba(255,190,77,.08)}";
  ...
  function run(action){
    ...
    if(action==="dock")return fwdStock("UNDOCK")||fwdStock("DOCK");
    if(action==="gate"||action==="stims"||action==="mine"||action==="destruct"){
      return fwdStock({gate:"WARPGATE",stims:"STIMS",mine:"MINE",destruct:"DESTRUCT"}[action]);
    }
  ...
  function paint(){
    ...
    var actions=[
      {id:"dock",lb:undock?"Undock":"Dock",ico:"dock",fwd:1,dim:!hasSec},
      {id:"warp",lb:"Warp",ico:"warp",hk:m.warp||"S-Sp",on:pickMode==="warp",dim:picking&&pickMode!=="warp"},
      {id:"swarp",lb:"Subwarp",ico:"swarp",hk:m.subwarp||"Sp",on:pickMode==="swarp",dim:picking&&pickMode!=="swarp"},
      {id:"gate",lb:"Warp Gate",ico:"gate",fwd:1,dim:!hasSec},
      {id:"stims",lb:"Stims",ico:"stims",fwd:1,dim:!hasSec},
      {id:"atk",lb:"Attack",ico:"atk",hk:m.attack||"A",dim:busy||!hasAtk},
      {id:"mine",lb:"Mine",ico:"mine",fwd:1,dim:busy||!hasSec},
      {id:"scan",lb:"Scan",ico:"scan",hk:m.scan||"C",dim:busy},
      {id:"destruct",lb:"Destruct",ico:"destruct",fwd:1,cls:"danger",dim:!hasSec}
    ];
    actions.forEach(function(a){
      var b=document.createElement("button");b.type="button";
      b.className="sa-tile"+(a.cls?" "+a.cls:"")+(a.dim?" dim":"")+(a.on?" on":"")+(captureAction===a.id?" capturing":"");
      b.dataset.act=a.id;
      b.innerHTML=ico(a.ico)+'<span>'+a.lb+'</span>'+(a.hk||captureAction===a.id?'<span class="hk">'+(captureAction===a.id?"…":a.hk)+"</span>":"");
...
;document.documentElement.appendChild(st)}
  ...
  function compactTiles(){
    var grid=document.querySelector('[class*="supportTileGrid"]');if(!grid)return;
    var tiles=grid.querySelectorAll('[class*="supportTile_"]');
    for(var i=0;i<tiles.length;i++){
      var t=tiles[i];var txt=t.textContent||"";
      var all=nums(txt);var cur=all.length?all[0]:0;
      var cap=0;var mi=txt.toUpperCase().indexOf("MAX");
      if(mi>=0){var mn=nums(txt.slice(mi));if(mn.length)cap=mn[0]}
      ...
      if(!bar){
        ...
        p.style.cssText="position:absolute;right:6px;bottom:5px;font:700 8px Orbitron,sans-serif;letter-spacing:.06em;color:rgba(255,190,77,.9);pointer-events:none;z-index:2";
        t.appendChild(p);
      }
      ...
      var p2=t.querySelector("[data-sa-pct]");if(p2)p2.textContent=cap>0?Math.round(pct*100)+"%":"";
    }
  }
