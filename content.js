...
window.__SA_CONFIRM_FLY__=window.__SA_CONFIRM_FLY__||function(opts){return new Promise(function(resolve){try{var prev=document.getElementById("sa-fly-modal");if(prev)prev.remove();var mode=String(opts&&opts.mode||"subwarp"),label=String(opts&&opts.label||"destination"),fleet=String(opts&&opts.fleet||"Fleet");var wrap=document.createElement("div");wrap.id="sa-fly-modal";wrap.style.cssText="position:fixed;inset:0;z-index:1000002;display:flex;align-items:center;justify-content:center;background:rgba(4,8,16,.62);backdrop-filter:blur(4px);font-family:ui-sans-serif,system-ui,sans-serif";var card=document.createElement("div");card.style.cssText="width:min(420px,92vw);background:linear-gradient(165deg,rgba(13,33,55,.98),rgba(10,15,25,.98));border:1px solid rgba(0,229,255,.45);border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.65),0 0 40px rgba(0,229,255,.12);overflow:hidden";var modeColor=mode==="warp"?"#67e8f9":"#86efac";var modeIcon=mode==="warp"?"🌀":"➡️";var modeWord=mode==="warp"?"Warp":"Subwarp";card.innerHTML='<div style="padding:12px 16px;border-bottom:1px solid rgba(0,229,255,.2);display:flex;align-items:center;gap:10px;background:rgba(0,229,255,.08)"><span style="font-size:18px">'+modeIcon+'</span><div><div style="color:#00e5ff;font-weight:800;letter-spacing:.08em;font-size:12px;text-transform:uppercase">Movement order</div><div style="color:#94a3b8;font-size:11px;margin-top:2px">'+fleet.replace(/[<>&]/g,"")+'</div></div></div><div style="padding:18px 16px 8px;color:#e2e8f0"><div style="font-size:13px;line-height:1.45">Fly <b style="color:'+modeColor+'">'+modeWord+'</b> to</div><div style="margin-top:8px;padding:10px 12px;border-radius:8px;background:rgba(0,0,0,.35);border:1px solid rgba(148,163,184,.25);color:#f8fafc;font-weight:700;font-size:14px;word-break:break-word">'+String(label).replace(/[<>&]/g,"")+'</div><div style="margin-top:12px;color:#64748b;font-size:11px;line-height:1.4">Tip: <b style="color:#94a3b8">Shift+click</b> skips this prompt and launches immediately.</div></div><div style="display:flex;gap:8px;padding:14px 16px 16px;justify-content:flex-end"><button type="button" data-a="no" style="min-width:96px;padding:10px 14px;border-radius:8px;border:1px solid rgba(148,163,184,.4);background:transparent;color:#94a3b8;font-weight:700;cursor:pointer">Cancel</button><button type="button" data-a="yes" style="min-width:130px;padding:10px 14px;border-radius:8px;border:1px solid rgba(0,229,255,.55);background:rgba(0,229,255,.16);color:#00e5ff;font-weight:800;cursor:pointer">Start '+modeWord+'</button></div>';function done(v){try{wrap.remove()}catch(_e){}resolve(!!v)}card.querySelector('[data-a="no"]').onclick=function(){done(false)};card.querySelector('[data-a="yes"]').onclick=function(){done(true)};wrap.onclick=function(e){if(e.target===wrap)done(false)};wrap.addEventListener("keydown",function(e){if(e.key==="Escape")done(false);if(e.key==="Enter")done(true)});wrap.appendChild(card);document.body.appendChild(wrap);setTimeout(function(){try{card.querySelector('[data-a="yes"]').focus()}catch(_e){}},30)}catch(_e){resolve(true)}})};
...
window.__SA_WING_CONFIRM__=window.__SA_WING_CONFIRM__||function(opts){
  try{
    ...
    wrap.style.cssText="position:fixed;inset:0;z-index:1000005;display:flex;align-items:center;justify-content:center;background:rgba(4,8,16,.7);backdrop-filter:blur(4px);font-family:Orbitron,ui-sans-serif,system-ui,sans-serif";
    var card=document.createElement("div");
    card.style.cssText="width:min(420px,92vw);background:linear-gradient(165deg,#1a1610,#0c0a08);border:1px solid rgba(255,190,77,.45);border-radius:4px;box-shadow:0 20px 60px rgba(0,0,0,.65);overflow:hidden;color:#f4f0e0";
    ...
    card.innerHTML='<div style="padding:12px 14px;border-bottom:1px solid rgba(255,190,77,.25);background:rgba(255,190,77,.08)"><div style="font-size:11px;font-weight:800;letter-spacing:.14em;color:#ffbe4d;text-transform:uppercase">'+String(mode).toUpperCase()+' · '+String(name).replace(/[<>&]/g,"")+'</div><div style="margin-top:6px;font-size:13px;font-weight:700">→ '+String(destLabel).replace(/[<>&]/g,"")+'</div></div><div style="padding:12px 14px;max-height:40vh;overflow:auto"><div style="font-size:9px;letter-spacing:.12em;color:#9a8;margin-bottom:6px">READY ('+ready.length+')</div>'+readyHtml+'<div style="font-size:9px;letter-spacing:.12em;color:#9a8;margin:12px 0 6px">BLOCKED ('+blocked.length+') · stock preflight may skip more</div>'+blockedHtml+'</div><div style="display:flex;gap:8px;padding:12px 14px;justify-content:flex-end;border-top:1px solid rgba(255,190,77,.2)"><button type="button" data-a="no" style="padding:10px 14px;border:1px solid rgba(255,255,255,.2);background:transparent;color:#bbb;font:700 10px Orbitron,sans-serif;letter-spacing:.1em;cursor:pointer">CANCEL</button><button type="button" data-a="yes" style="padding:10px 14px;border:1px solid rgba(255,190,77,.55);background:rgba(255,190,77,.15);color:#ffbe4d;font:700 10px Orbitron,sans-serif;letter-spacing:.1em;cursor:pointer"'+(ready.length?"":" disabled")+'">GO READY ('+ready.length+')</button></div>';
    function done(go){
...
window.__SA_ACTION_BAR__=window.__SA_ACTION_BAR__||(function(){
  ...
  function css(){
    var cssTxt=""
+"#sa-action-bar{position:fixed;left:50%;bottom:16px;transform:translateX(-50%);z-index:999990;display:flex;flex-direction:column;align-items:center;gap:6px;pointer-events:none;"
...
+"#sa-action-bar .sa-cancel-row{pointer-events:auto;display:none;align-items:center;justify-content:center;min-height:0}"
...
+"#sa-action-bar .sa-tile{appearance:none;width:78px;min-height:72px;padding:8px 4px 6px;cursor:pointer;border-radius:0;"
...
+"#sa-combat-log-box .sa-cl-ico:hover{border-color:rgba(255,190,77,.5);color:#ffbe4d;background:rgba(255,190,77,.08)}";
  ...
  function selectWing(id,ev){
    ...
    if(multi){
      // Ctrl/Cmd+click: chain-toggle this wing into the selection
      if(idx>=0)activeWingIds=activeWingIds.filter(function(x){return x!==id});
      else activeWingIds=activeWingIds.concat([id]);
    }else{
      // plain click: solo select, or clear if already sole selection
      if(idx>=0&&activeWingIds.length===1){clearSelection();return}
      activeWingIds=[id];
    }
    try{
  ...
  function ico(name){
    var sv={
      ...
      atk:'<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M8 8 L24 16 L8 24 Z"/></svg>',
  ...
  function run(action){
    ...
    if(action==="subwarp"||action==="swarp"){
      if(activeWingIds.length&&window.__SA_WING_PICK__){
        ...
        if(has){window.__SA_WING_PICK__.start(activeWingIds.slice(),"subwarp");paint();return!0}
      }
    ...
    if(action==="warp"){
      if(activeWingIds.length&&window.__SA_WING_PICK__){
        if(selectedFleetCount()>0){window.__SA_WING_PICK__.start(activeWingIds.slice(),"warp");paint();return!0}
      }
    ...
    if(action==="attack"||action==="atk"){
      if(activeWingIds.length){try{window.__SA_FLIGHT_LOG__.push({type:"WARN",msg:"ATK is single-fleet — clear wings first"})}catch(e){}
        return!1}
      return clickLabel(/^\s*attack\s*$/i);
    }
    ...
    var wrap=document.createElement("div");wrap.id="sa-groups-board";
  ...
  function ensure(){
    ...
    (function(){
      ...
      function savePos(){
        try{localStorage.setItem(POS_KEY,JSON.stringify({left:parseFloat(root.style.left),top:parseFloat(root.style.top)}))}catch(e){}
      }
      function resetPos(){
      ...
      try{
        ...
        if(raw){var p=JSON.parse(raw);if(Number.isFinite(p.left)&&Number.isFinite(p.top))applyPos(p.left,p.top)}
      }catch(e){}
      ...
      window.addEventListener("resize",function(){if(root.classList.contains("sa-bar-pos"))clamp()});
    })();
    return root;
  }
  function paint(){
    if(!isBarVisible()){if(root&&root.isConnected)root.style.display="none";return}
    var bar=ensure();
    ...
    for(var i=0;i<5;i++){
      ...
      b.title=g?(g.name||("Wing "+(i+1)))+" · key "+(i+1)+(n?" · Ctrl/Cmd+click to chain":""):"Empty wing slot · Edit Wings";
    ...
    var actions=[
      {id:"warp",lb:"Warp",ico:"warp",hk:m.warp||"S-Sp",on:pickMode==="warp",dim:picking&&pickMode!=="warp"},
      {id:"swarp",lb:"Swarp",ico:"swarp",hk:m.subwarp||"Sp",on:pickMode==="swarp",dim:picking&&pickMode!=="swarp"},
      {id:"scan",lb:"Scan",ico:"scan",hk:m.scan||"C",dim:activeWingIds.length>0||picking},
      {id:"atk",lb:"Atk",ico:"atk",hk:m.attack||"A",dim:activeWingIds.length>0||picking}
    ];
    actions.forEach(function(a){
      var b=document.createElement("button");b.type="button";
      b.className="sa-tile"+(a.cls?" "+a.cls:"")+(a.dim?" dim":"")+(a.on?" on":"")+(captureAction===a.id?" capturing":"");
      b.innerHTML=ico(a.ico)+'<span>'+a.lb+'</span><span class="hk">'+(captureAction===a.id?"…":a.hk)+'</span>';
      b.onclick=function(){if(a.dim)return;run(a.id)};
