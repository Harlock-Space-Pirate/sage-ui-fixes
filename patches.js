...
globalThis.__SA_PATCHES__ = [
  ...
  },
  {
    ...
    replace:
      'console.error("Failed to attack starbase:",Ch);const _errStr=String(Ch?.message||Ch?.stack||JSON.stringify(Ch)||"");if(/1367933016|0x51890058|Starbase contested/i.test(_errStr)){Pt(`🛡️ ${To.systemName} starbase is CONTESTED and under protection/cooldown! (0x51890058)`,"error",{title:`Starbase Contested — ${To.systemName}`,targets:[_y(To)]})}else if(/Not within range/i.test(_errStr)){Pt("📡 Fleet is not in range of that starbase. Idle on the target system first.","error",{title:"Out of range",targets:[_y(To)]})}else if(/npc_attacker_ownership_missing/i.test(_errStr)){Pt("This hull has no Jorvik/Baron faction tag. Tag it on console.leeks.ink (Fleet tab) then retry.","error",{title:"No faction tag",targets:[_y(To)]})}else if(/1367933091|0x51890023|resource=ap/i.test(_errStr)){Pt("⚡ Insufficient AP! Reload fleet AP or top up ammo.","error",{title:"AP Depleted",targets:[_y(To)]})}else{Pt(xp(Ch),"error",{title:"Starbase attack failed",targets:[_y(To)]})}',
  },
  {
    id: "attack-fl-error",
